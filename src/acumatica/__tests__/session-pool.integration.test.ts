...
describeRedis('SessionPool (Redis integration)', () => {
  ...
  // Pool keys live under the per-(baseUrl, account, company) prefix — track the
  // real keyPrefix/lockoutKey per pool, NOT the legacy account-only pattern
  // (which matches nothing and leaked test keys between runs).
  const trackedPools: Array<{ keyPrefix: string; lockoutKey: string }> = [];
  ...
  afterEach(async () => {
    ...
    for (const { keyPrefix, lockoutKey } of trackedPools) {
      const keys = await cleanupRedis.keys(`${keyPrefix}:*`);
      if (keys.length > 0) await cleanupRedis.del(...keys);
      await cleanupRedis.del(lockoutKey);
    }
    trackedPools.length = 0;
    await cleanupRedis.quit();
  });

  function trackPool(pool: SessionPool): SessionPool {
    trackedPools.push({ keyPrefix: pool.keyPrefix, lockoutKey: pool.lockoutKey });
    return pool;
  }

  // Warm the pool's Redis connection before issuing any command. enableOfflineQueue
  // is false, so a command issued before the socket is 'ready' degrades the pool —
  // that race is exactly why these tests were flaky (a freshly-constructed test
  // pool's first checkout can beat the connect). In prod the pool is always warm.
  async function ready(pool: SessionPool): Promise<SessionPool> {
  ...
  it('should checkout/checkin round-trip via Lua scripts', async () => {
    const pool = await ready(trackPool(makePool()));
    ...
    const slotIds = await cleanupRedis.smembers(`${pool.keyPrefix}:slots`);
    ...
    const slotData = await cleanupRedis.hgetall(`${pool.keyPrefix}:slot:${handle.slotId}`);
  ...
  it('should create multiple slots up to maxSize', async () => {
    const pool = await ready(trackPool(makePool({ maxSize: 3 })));
    let callCount = 0;
  ...
  it('should enforce backpressure when at capacity', async () => {
    const pool = await ready(
      trackPool(makePool({ maxSize: 1, checkoutTimeoutMs: 500, pollIntervalMs: 50 })),
    );
  ...
  it('should reclaim stale checkouts via Lua script', async () => {
    const pool = await ready(
      trackPool(makePool({ maxSize: 1, staleCheckoutMs: 100, checkoutTimeoutMs: 2000, pollIntervalMs: 50 })),
    );
  ...
  it('should evict slot and remove from Redis', async () => {
    ...
    const result = await pool.withSession(async (handle) => {
      const slotsBefore = await cleanupRedis.smembers(`${pool.keyPrefix}:slots`);
  ...
  it('convergeIdleToMaxSize sheds excess idle slots via EVICT_IDLE_LUA down to maxSize', async () => {
    ...
    const pool = trackPool(makePool({ maxSize: 2, keepaliveMs: 50, convergeIdleToMaxSize: true }));
